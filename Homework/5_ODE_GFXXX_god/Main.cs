...
public class Program {
    public static void Main() {
        ...
        using (var outtxt = new StreamWriter("Out.txt")) {
            outtxt.WriteLine("--- PART A ---\n");
            outtxt.WriteLine("-- Testing the routines by solving the ordinary differential equations (ODE) ---\n");
            outtxt.WriteLine("In sho_data.txt and sho_plot.png is the data and plot, respectively, from solving: u''=-u");
            outtxt.WriteLine("In exp_data.txt and exp_plot.png is the data and plot, respectively, from solving: dy/dx = y");
            outtxt.WriteLine("In poly_data.txt and poly_plot.png is the data and plot, respectively, from solving: y'' = 6x");
            outtxt.WriteLine("For comparison, the plots also contain the analytical solutions.\n");

            outtxt.WriteLine("-- Reproduce (with my routines) the example from the scipy.integrate.odeint manual (oscillator with friction) ---\n");
            outtxt.WriteLine("In oscillator_with_friction.txt and oscillator_with_friction_plot.png is the data and plot, respectively.\n");

            outtxt.WriteLine("-- Reproduce (with my routines) the example from the the scipy.integrate.solve_ivp manual (Lotka-Volterra system) ---\n");
            outtxt.WriteLine("In lotka_volterra.txt and lotka_volterra_plot.png is the data and plot, respectively.\n");

            outtxt.WriteLine("--- PART B ---\n");
            outtxt.WriteLine("-- Integrate this equation with ε=0 and initial conditions u(0)=1, u'(0)=0 : (Newtonian circular motion). --\n");
            outtxt.WriteLine("The resulting data is in Newtonian_circular_motion.txt\n");

            outtxt.WriteLine("-- Integrate this equation with ε=0 and initial conditions u(0)=1, u'(0)≈-0.5 : ( Newtonian elliptical motion). --\n");
            outtxt.WriteLine("The resulting data is in Newtonian_elliptical_motion.txt\n");

            outtxt.WriteLine("-- Integrate this equation with ε≈0.01 and initial conditions u(0)=1, u'(0)≈-0.5 : (Relativistic precession of a planetary orbit). --\n");
            outtxt.WriteLine("The resulting data is in Relativistic_motion.txt\n");

            outtxt.WriteLine("--  Plot planetary orbit for Newtonian circular motion, Newtonian elliptical motion, and Relativistic. --\n");
            outtxt.WriteLine("This plot is in Planetary_orbits_plot.png\n");

            outtxt.WriteLine("--- Part C ---\n");
            outtxt.WriteLine("-- Using my numerical ODE integrator reproduce the solution the remarkable stable planar periodic solution to the three-body problem --\n");
            outtxt.WriteLine("The reproduced solution data is in three_body_problem.txt and plotted in three_body_problem_plot.png\n");
        }
        ...
        Console.WriteLine("In lotka_volterra.txt and lotka_volterra_plot.png is the data and plot, respectively, from solving: Lotka-Volterra system");
        ...
        Console.WriteLine("In oscillator_with_friction.txt and oscillator_with_friction_plot.png is the data and plot, respectively, from solving: damped oscillator");
        ...
        // Part B: Save planetary orbit data
        double phi_max = 12 * PI;
        vector u0 = new vector(new double[]{1.0, 0.0});
        var (xs_nc, ys_nc) = OdeSolver.driver(ODEModels.Precession(0.0), 0, u0, phi_max, acc:1e-6, eps:1e-6);
        using (var writer = new StreamWriter("Newtonian_circular_motion.txt")) {
            for (int i = 0; i < xs_nc.Count; i++) writer.WriteLine($"{xs_nc[i]} {ys_nc[i][0]}");
        }
